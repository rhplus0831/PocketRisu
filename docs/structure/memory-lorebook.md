...
- `src/ts/process/lorebook.svelte.ts` — approximately 760 lines. Owns lorebook creation, matching, recursive activation, decorators, token-budget selection, and JSON import/export.
...
- `src/ts/process/memory/hypamemoryv2.ts` — approximately 501 lines. Metadata-aware embedding processor used by the experimental Hypa V3 implementation.
...
- `src/ts/process/memory/taskRateLimiter.ts` — approximately 188 lines. Queue used for embedding and experimental summarization requests.
...
- `src/ts/process/memory/hypav3.ts` — approximately 2,058 lines. Long-term history compression, summary retrieval, summarization-model dispatch, presets, and serialization.
...
- `src/ts/process/embedding/addinfo.ts` — approximately 34 lines. Retrieval-augments a character description from `character.additionalText`.
...
- `src/ts/process/modules.ts` — approximately 590 lines. Defines module packages, import/export, runtime scope resolution, and projection of active module content.
...
- For regex lore, every key must use `/pattern/flags` syntax. A match is tested against raw message data (`src/ts/process/lorebook.svelte.ts:144`).
...
- Regex lore parsing expects slash-delimited patterns with trailing flags. Invalid expressions return no match rather than surfacing an error (`src/ts/process/lorebook.svelte.ts:144`).
...
- `src/lib/Others/HypaV3Modal.svelte` and its sibling components provide manual summary editing, re-summarization, categories, tags, important flags, and display metrics.
- `src/ts/process/scripts.ts`, `src/ts/process/triggers.ts`, and `src/ts/process/mcp/` consume module regex, trigger, and MCP projections.
- `src/ts/characterCards.ts` and `src/ts/interchangeability.ts` translate lorebooks/modules to and from character-card representations.
- `src/lib/SideBars/LoreBook/` owns lorebook editing, folders, drag-and-drop, and per-chat child activation UI.
