...
| `server/node/assetMaintenanceLock.cjs`, `assetDedup.cjs` | Cross-process exclusion shared by live asset mutations, destructive import swaps, journal recovery, and the controlled multi-instance dedup worker. Ancestor symlinks canonicalize to one identity; the swappable `assets` leaf itself may not be a symlink. Lock state lives beside that directory; dedup uses strict target validation, locale-independent UTF-8 byte ordering, byte verification, link-to-hidden-temp, final inode/content revalidation, atomic rename, and directory fsync. |
