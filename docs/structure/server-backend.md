...
- Storage mutations run through the promise-based `queueStorageOperation()` serialization point at `server/node/server.cjs:159`, wrapped by `queueStorageMutation()` — the import barrier drains that queue before an import claims it, so a mutation either commits entirely before the import's `BEGIN` or is refused with `503 IMPORT_IN_PROGRESS` plus `Retry-After` instead of joining (and then being rolled back with) the import transaction. Every KV/chat-row/asset write must use `queueStorageMutation()`, never `queueStorageOperation()` directly.
