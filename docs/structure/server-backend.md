...
- With the browser resource cache disabled, a capable server uses `GET /api/db/read-raw-for-boot`. With it enabled, the client verifies resident IndexedDB segments, advertises up to 8,192 hashes grouped as root/characters/botPresets/modules/personas, and calls `POST /api/db/read-cached`. The raw route returns verbatim bytes for recovery, HTTP 204 only for explicit absence, and never uses a route-ambiguous 404.
- The cached-read endpoint flushes pending writes, prepares the same stubs-only view and ETag as `/api/read`, MessagePack-encodes each group member separately, and returns `{hash}` for advertised hits or `{bytes}` for misses. Any malformed envelope, missing/corrupt local entry, unadvertised hit, or ETag disagreement makes a capable client retry the raw boot read.
- Fresh initialization calls `POST /api/db/create-if-absent`, not generic replacement. The route runs behind the writer-session check and serialized storage mutation queue, creates the canonical empty database in a SQLite transaction only if the live key is absent, returns 201 plus the committed ETag to the winner, and returns a definitive 409 without mutation to concurrent or stale readers. A losing bootstrap rereads the winner's database.
...
| Authentication/session | `GET /api/test_auth`, `POST /api/login`, `POST /api/token/refresh`, `POST /api/session`, `POST /api/set_password`, and `POST /api/crypto`. `/api/session` also advertises database and plugin-storage protocol capabilities. | `NodeStorage` auth lifecycle and capability negotiation; password hashing near the end of that file. |
...
- Optimized plugin publication is generation/manifest-bound. The database mode and `pluginStorageGeneration`, exact manifest, value/owner rows, quota state, and recovery-dirty token publish atomically through specialized endpoints. Generic KV routes and JSON Patch cannot mutate those optimized roots or controls. A database patch may update only the inline value/owner maps after an authoritative inline-mode check. Production mode changes use a private staged transition and one final transaction. See [Plugin storage](plugin-storage.md).
...
- To change database boot/read/write synchronization, inspect `/api/session`, `/api/read`, `/api/db/read-cached`, `/api/db/read-raw-for-boot`, `/api/db/create-if-absent`, `server/node/dbCachedRead.cjs`, `/api/write`, and `NodeStorage.readDatabaseForBoot()`/`createDatabaseIfAbsent()` together.
