...
- Portable and server backups never contain `chats/` entries. Each endpoint flushes pending data and pins a chunk-aware read-only WAL transaction while still inside the storage queue, then releases the queue and uses that immutable reader for database assembly, KV enumeration, and KV-backed assets. `buildSelfContainedBackupDatabase()` returns a temporary `{ filePath, size }` spool rather than a monolithic Buffer. `streamRisuSaveToFile()` writes the legacy magic header plus standard MessagePack incrementally, merging each stub with one decoded chat row and releasing it before the next. A missing referenced chat aborts an export instead of serializing a bare stub.
