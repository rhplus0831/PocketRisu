...
`Status` column uses `Open`, `Fixed`, or `Deferred`; 48 findings are currently
`Open`, 11 are `Fixed`, and 1 is `Deferred`. Per-document severities are 15 High, 38 Medium, and 7 Low, but this
...
| Finding | Status | Doc sev. | What could be lost | Trigger / likelihood |
|---|---|---|---|---|
| [Backups omit drafts and imports delete them](audit/backups-omit-drafts-and-imports-delete-them.md) | Open | Medium | Every unsent composer draft | Every backup restore/import clears the `drafts/` prefix; no backup path includes it |
...
| Finding | Status | Doc sev. | What could be lost | Trigger / likelihood |
|---|---|---|---|---|
| [Docker server backups are ephemeral](audit/docker-server-backups-are-ephemeral.md) | Open | Medium | Every server-side `.bin` backup | Every image update or container recreation with the shipped Compose file — discovered exactly when a backup is needed |
...
| [Global chat budget evicts newer bundles before older loose versions](audit/global-chat-budget-evicts-newer-bundles-before-older-loose-versions.md) | Open | Medium | 25 newer recovery points destroyed where one older loose file would have sufficed | A global byte-cap overage across chats |
