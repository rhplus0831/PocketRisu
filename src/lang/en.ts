...
export const languageEnglish = {
    ...
    playground: {
        ...
        inlayDeleteReferenced: "Skipped {count} asset(s) because stored chat messages still reference them.",
        ...
        inlayScanFailed: "Could not scan stored chat messages. No assets were classified as unreferenced.",
    },
    confirm: "Confirm",
    yes: "Yes",
    no: "No",
    ...
    resourceCache: "Browser resource cache",
    resourceCacheDesc: "Reuse verified database, chat, and plugin data stored on this device for faster starts and fewer downloads. Disabling this clears all cached data.",
    resourceCacheAnnouncement: "PocketRisu can keep a verified local cache of unchanged data. This makes future starts faster and uses less data. You can enable or disable it later in Advanced Settings; disabling clears the cached data.",
    resourceCacheAnnouncementEnable: "Enable cache",
    resourceCacheAnnouncementNotNow: "Not now",
    resourceCacheEnabled: "Enabled",
    resourceCacheDisabled: "Disabled",
    resourceCacheUsage: (entries: number, size: string) => `${entries.toLocaleString()} entries · ${size}`,
    resourceCacheEntries: "Entries",
    resourceCacheManifests: "Resource manifests",
    resourceCacheSize: "Total size",
    resourceCacheDashboardDesc: "Disposable, verified data stored in this browser to reduce repeated downloads.",
    resourceCacheClear: "Clear browser cache",
    resourceCacheClearConfirm: "Clear all cached browser data? It will be rebuilt as needed.",
    resourceCacheCleared: "Browser cache cleared",
    ...
    saveBackupForMain: "Export for PocketRisu Main Rollback",
    saveBackupForMainConfirm: "This creates a non-destructive rollback archive for the PocketRisu main branch. Full chats, inline plugin values, ordinary assets, inlays, and cold storage are included. Unsent composer drafts and remembered MCP tool-call payloads are excluded because main cannot import those namespaces.\n\nRestore this archive into a fresh main data directory. Keep your existing serve data directory and a normal PocketRisu backup until you have verified the restored data.\n\nProceed?",
    ...
    activeTabChange: "Current tab is inactive because another tab is active.",
    writerTakeoverBody: "Another browser session took over write access. This page can no longer save changes. Stay here in read-only mode so you can copy your work, or discard this page's unsaved changes, reload, and take write access back from the other session.",
    ...
    clientUpgradeDirtyBody: "The server was updated and this client can no longer save changes. Stay here in read-only mode so you can copy unsaved work, or discard it and reload the new client bundle.",
    clientUpgradeOfflineBanner: "Read-only: the server requires a newer client. Copy any unsaved work before reloading.",
    clientUpgradeReloadFailed: "PocketRisu still has an outdated client after reloading. Clear this site's cache or check the reverse-proxy cache, then reload manually.",
    clientUpgradeInvalidResponse: "The server requires a client update, but did not provide valid build information. Reload manually after checking the server or proxy configuration.",
    ...
    resetPluginPermission: "Reset all to Ask",
    ...
    managePluginPermissions: "Manage plugin permissions",
    pluginPermissionsFor: "Permissions for \"{}\"",
    pluginPermissionEditorHint: "Changes are saved immediately. Reset all to Ask clears saved choices so the plugin prompts again.",
    pluginPermissionFetchLogs: "Read request logs",
    pluginPermissionDatabase: "Access the full database",
    pluginPermissionMainDom: "Access the main page",
    pluginPermissionReplacer: "Replace chat content",
    pluginPermissionProvider: "Act as an AI provider",
    pluginPermissionSendChat: "Send chat messages",
    pluginPermissionGranted: "Granted",
    pluginPermissionRevoked: "Revoked",
    pluginPermissionAsk: "Ask when requested",
    pluginPermissionGrant: "Grant",
    pluginPermissionRevoke: "Revoke",
    ...
    optimizePluginMemoryDesc: "Helps PocketRisu use less memory by loading saved plugin data only when it is needed. This may improve performance, especially if your plugins store a lot of data.",
    pluginCompatibilitySection: "Compatibility",
    pluginCompatibilityDisableConfirm: "Disabling this option will break compatibility with nearly all plugins. Disabling it is not recommended.",
    autoConvertPluginStorageValues: "Automatically convert compatible plugin values",
    autoConvertPluginStorageValuesDesc: "When an optimized write or storage-mode migration is not JSON-compatible, convert Date to an ISO string, Map to entry arrays, Set to an array, and BigInt to a decimal string. Undefined values and directly observed array holes are preserved by a versioned lossless codec; non-finite numbers become null. Functions and circular references are still rejected.",
    ...
    optimizePluginMemoryWorking: "Changing plugin storage mode… This may take a while.",
    ...
    pluginStorageRecoveryTitle: "Plugin storage recovery required",
    pluginStorageRecoveryDesc: "PocketRisu could not verify one or more plugin storage entries. No data was overwritten or deleted. Restoring a known-good restore point is the safest option. If none is available, you can manage the affected data manually. Diagnostics contain encoded keys only.",
    pluginStorageRecoveryRestorePoint: "Restore from restore point",
    pluginStorageRecoveryManage: "Manage affected data…",
    pluginStorageRecoveryCheckAgain: "Check again",
    pluginStorageRecoveryCopy: "Copy diagnostics",
    pluginStorageRecoveryRetrySuccess: "Plugin storage reconciliation completed. The quarantine is clear.",
    pluginStorageRecoveryStillAffected: (count: number) => `${count} affected plugin storage entr${count === 1 ? "y is" : "ies are"} still present. No data was changed.`,
    pluginStorageRecoveryLastChecked: (time: string) => `Last checked at ${time}.`,
    pluginStorageRecoveryCopySuccess: "Encoded-key diagnostics copied.",
    pluginStorageRecoveryCopyFailed: "Could not copy plugin storage diagnostics.",
    pluginStorageRecoveryManagerTitle: "Manage affected plugin data",
    pluginStorageRecoveryManagerDesc: "Download an exact copy before making changes. Inline recovery replaces the affected external row. Delete is offered only when no usable inline copy exists.",
    pluginStorageRecoveryManagerLoading: "Inspecting affected data…",
    pluginStorageRecoveryManagerLoadFailed: "Could not inspect the affected data. Check the server connection and try again.",
    pluginStorageRecoveryManagerOptimizedOnly: "Direct management is available for optimized plugin storage. Use a restore point for this recovery state.",
    pluginStorageRecoveryManagerEmpty: "No affected optimized plugin storage entries remain.",
    ...
    pluginStorageRecoveryExternalCopy: "External copy",
    pluginStorageRecoveryInlineCopy: "Inline copy",
    pluginStorageRecoveryAvailable: "available",
    pluginStorageRecoveryUnavailable: "unavailable",
    pluginStorageRecoveryDownload: "Download raw copy",
    pluginStorageRecoveryDownloadSuccess: "Downloaded the exact affected row.",
    pluginStorageRecoveryDownloadFailed: "Could not download the affected row. Refresh the recovery details and try again.",
    pluginStorageRecoveryUseInline: "Use inline copy",
    pluginStorageRecoveryUseInlineConfirm: (encodedKey: string) => `Replace the affected external row with its valid inline copy?\n\n${encodedKey}\n\nDownload the external row first if you may need it later.`,
    pluginStorageRecoveryUseInlineSuccess: "Replaced the affected external row with the inline copy.",
    pluginStorageRecoveryDelete: "Delete unrecoverable data",
    pluginStorageRecoveryDeleteConfirm: (encodedKey: string) => `Permanently delete this unrecoverable plugin storage entry?\n\n${encodedKey}\n\nThis cannot be undone. Download the raw copy first if you may need it later.`,
    pluginStorageRecoveryDeleteSuccess: "Deleted the unrecoverable plugin storage entry.",
    pluginStorageRecoveryActionFailed: "The recovery action did not complete. No unconfirmed action was retried.",
    pluginStorageRecoveryNoDirectAction: "This issue cannot be repaired directly. Restore a known-good restore point or check again after resolving server access.",
    pluginStorageRecoveryRefreshDetails: "Refresh details",
    ...
    backupRestoreCommittedFailure: "The server committed this backup restore but reported an error afterward. Dismiss this warning to reload the authoritative server state.",
    ...
    importSaveFolderCommittedFailure: "The server committed this save-folder import but reported an error afterward. Dismiss this warning to reload the authoritative server state.",
    importSaveFolderOutcomeUnknown: "The server may have committed this save-folder import, but its acknowledgement was lost or invalid. PocketRisu will not retry it. Dismiss this warning to reload and reconcile the authoritative server state.",
    importSaveFolderFailure: "The server confirmed that the save-folder import was not committed. Your current data remains active.",
    ...
    pluginStoragePageCount: (page: number, pages: number, keys: number) => `on page ${page} / ${pages} · ${keys} keys`,
    ...
    backupTabDesc: "Manage snapshots, full backups, chat backups, and local backups in one place.",

    backupSnapshot: "Snapshot (DB only)",
    backupSnapshotEmpty: "No snapshots.",
    backupSnapshotUnreadable: "Unreadable",
    ...
    chatBackupsDesc: "Browse automatic versions captured before chats were changed. Importing creates a new chat and does not overwrite the original.",
    chatBackupsLoading: "Loading chat backups...",
    chatBackupsEmpty: "No chat backups found.",
    chatBackupsRefresh: "Refresh chat backups",
    chatBackupsLoadFailed: "Failed to load chat backups",
    chatBackupUnnamedCharacter: "Unnamed character",
    chatBackupDeletedCharacter: (id: string) => `Deleted character · ${id}`,
    chatBackupDeletedChat: (id: string) => `Deleted chat · ${id}`,
    chatBackupVersionCount: (count: number) => count === 1 ? "1 version" : `${count} versions`,
    chatBackupNewest: (date: string) => `Newest: ${date}`,
    chatBackupVersionsLoading: "Loading versions...",
    chatBackupVersionsEmpty: "No versions found.",
    chatBackupReason: (reason: string) => `Reason: ${reason}`,
    chatBackupImportAsNew: "Import as new chat",
    chatBackupImportIntoOriginal: (name: string) => `Import into ${name}`,
    chatBackupImportAnother: "Import into another character",
    chatBackupSelectTarget: "Choose the character that will receive the restored chat.",
    chatBackupNoCharacters: "Create a character before importing a chat backup.",
    chatBackupNoOtherCharacters: "There are no other characters to import into.",
    chatBackupVersionMissing: "That chat backup version is no longer available.",
    chatBackupImportSuccess: (name: string) => `Imported "${name}" as a new chat.`,
    chatBackupImportFailed: "Failed to import chat backup",
    chatBackupRestoredSuffix: (date: string) => `(restored ${date})`,
    chatBackupUntitledChat: "Untitled chat",
    chatBackupTargetMissing: "The selected character no longer exists.",
