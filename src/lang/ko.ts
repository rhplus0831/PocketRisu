...
export const languageKorean = {
  ...
  resourceCache: "브라우저 리소스 캐시",
  resourceCacheDesc: "이 기기에 저장된 검증된 데이터베이스, 채팅 및 플러그인 데이터를 재사용해 시작 속도를 높이고 다운로드를 줄입니다. 비활성화하면 캐시된 데이터가 모두 삭제됩니다.",
  resourceCacheAnnouncement: "PocketRisu가 변경되지 않은 데이터를 검증된 로컬 캐시에 보관할 수 있습니다. 이후 시작 속도가 빨라지고 데이터 사용량이 줄어듭니다. 고급 설정에서 나중에 켜거나 끌 수 있으며, 비활성화하면 캐시 데이터가 삭제됩니다.",
  resourceCacheAnnouncementEnable: "캐시 사용",
  resourceCacheAnnouncementNotNow: "나중에",
  resourceCacheEnabled: "사용 중",
  resourceCacheDisabled: "사용 안 함",
  resourceCacheUsage: (entries: number, size: string) => `${entries.toLocaleString()}개 항목 · ${size}`,
  resourceCacheEntries: "항목",
  resourceCacheManifests: "리소스 매니페스트",
  resourceCacheSize: "전체 크기",
  resourceCacheDashboardDesc: "반복 다운로드를 줄이기 위해 이 브라우저에 저장된 폐기 가능한 검증 데이터입니다.",
  resourceCacheClear: "브라우저 캐시 비우기",
  resourceCacheClearConfirm: "브라우저에 캐시된 데이터를 모두 삭제할까요? 필요할 때 다시 생성됩니다.",
  resourceCacheCleared: "브라우저 캐시를 비웠습니다",
  ...
  pluginPermissionReplacer: "채팅 내용 교체",
  pluginPermissionProvider: "AI Provider로 동작",
  pluginPermissionSendChat: "채팅 메시지 전송",
  pluginPermissionGranted: "허용됨",
  pluginPermissionRevoked: "철회됨",
  pluginPermissionAsk: "요청 시 확인",
  pluginPermissionGrant: "허용",
  pluginPermissionRevoke: "철회",
  pluginV2Warning:
  ...
  autoConvertPluginStorageValuesDesc:
    "최적화 저장 값이 JSON과 호환되지 않으면 Date는 ISO 문자열, Map은 항목 배열, Set은 배열, BigInt는 10진수 문자열로 변환하고 undefined, 유한하지 않은 숫자, 배열의 빈 항목은 null로 변환합니다. 함수와 순환 참조는 계속 거부됩니다.",
  legacyPluginCompatibility: "레거시 플러그인 호환성",
  legacyPluginCompatibilityDesc:
    "레거시 플러그인에 더 긴 종료 시간을 제공하고 onUnload 중 상태, UI, 네트워크, 에셋 및 저장소의 제한된 마무리 작업을 허용하며, 종료 단계만 실패한 경우 경고 후 계속합니다. 새 등록, 모델/채팅 작업, 플러그인 교체 및 비어 있지 않은 UI 생성은 계속 차단됩니다.",
  legacyPluginCompatibilityTeardownWarning:
    "하나 이상의 플러그인이 정상적으로 종료되지 않았습니다. 호환성 모드가 해당 플러그인을 종료하고 다시 불러오기를 계속했습니다.",
  ...
  optimizePluginMemoryLegacyOff:
    "메모리 최적화가 켜져 있는 동안 V2/V2.1 플러그인은 비활성 상태로 유지됩니다. 사용하려면 최적화를 끄세요.",
  optimizePluginMemoryLegacyAutoDisabled: (plugins: string) =>
  ...
  pluginRuntimeFailed: (plugin: string) =>
    `플러그인 "${plugin}"이(가) 예기치 않게 중지되었습니다.`,
  ...
  backupRestoreCommittedFailure:
    "서버가 백업 복원을 커밋했지만 이후 오류를 보고했습니다. 이 경고를 닫으면 권한 있는 서버 상태를 다시 불러옵니다.",
  ...
  pluginStoragePreviousPage: "이전",
  pluginStorageNextPage: "다음",
  pluginStoragePageCount: (page: number, pages: number, keys: number) => `${page} / ${pages} 페이지 · 키 ${keys}개`,
